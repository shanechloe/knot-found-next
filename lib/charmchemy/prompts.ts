...
export function buildImageGenerationPrompt(input: {
  ...
  return [
    'Create one realistic jewelry product photo from the uploaded reference image.',
    'The uploaded image is the only material source.',
    '',
    'Hard rules:',
    '- Use only materials clearly visible in the uploaded image and listed in approved inventory.',
    '- Do not invent or add any new beads, chains, clasps, spacer beads, pendants, pearls, charms, gemstones, wires, jump rings, connectors, findings, props, packaging, textures, or colors.',
    '- Do not recolor, reshape, resize, simplify, polish, stylize, or reinterpret any visible material.',
    '- The finished piece must look like the exact uploaded materials were physically rearranged.',
    '- Material accuracy is more important than beauty, creativity, symmetry, luxury styling, or editorial aesthetics.',
    '',
    'Especially forbidden unless explicitly visible in approved inventory:',
    '- Gold chains',
    '- Gold spacer beads',
    '- Leaf charms',
    '- Star charms',
    ...
    input.previousViolations && input.previousViolations.length > 0
      ? `Previous attempt violations to explicitly forbid: ${previousViolations.join('; ')}.`
      : '',
    '',
